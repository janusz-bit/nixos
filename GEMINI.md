...
* **OpenCode**: Declarative configuration inline in `modules/overlays/opencode.nix` (Nix overlay generating `opencode.json` + `web-search-mcp.py` at build time). Default model: `ollama-cloud/glm-5.2:cloud`. Provider `ollama` (OpenAI-compatible, `http://localhost:11434/v1`) with model `orinth:35b`. Provider `ollama-cloud` (OpenAI-compatible, `https://ollama.com/v1`, apiKey from `{env:OLLAMA_API_KEY}`) with model `glm-5.2:cloud`. Local MCP servers: `web_search_and_fetch` via `uv run` (Ollama web_search/web_fetch API) and `nixos` via `pkgs.mcp-nixos` (`lib.getExe`; NixOS/Home Manager/nixpkgs/flakes/wiki search tools). Installed on all hosts through the `base/opencode` module.
...
* **Agenix** (`modules/hosts/base/agenix.nix`): Exports `CACHIX_AUTH_TOKEN`, `GITHUB_TOKEN`, `OLLAMA_API_KEY`, `GOOGLE_API_KEY`, and `NIX_CONFIG` (access-tokens for `github.com`) via `environment.shellInit` from age-encrypted secrets.

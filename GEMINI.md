...
* **Dynamiczne VFIO** (`modules/hosts/nixos/vfio.nix`): przekazanie dGPU NVIDIA (`0000:01:00.0` karta + `0000:01:00.1` HDMI audio) do VM **bez restartu systemu**. IOMMU przez `boot.kernelParams` (`intel_iommu=on`, `iommu=pt`, `kvm.ignore_msrs=1`), `virtualisation.libvirtd` (qemu bez roota + grupa `kvm`, udev rule dla `/dev/vfio/*`, swtpm, `security.polkit.enable`), QEMU hook `vfio-nvidia` (`virtualisation.libvirtd.hooks.qemu`; instalowany do `/var/lib/libvirt/hooks/qemu.d/`). Hook przy `prepare` VM `win11`: zatrzymuje `display-manager` i `ollama`, ubija procesy trzymające GPU (`lsof /dev/nvidia*` + `/dev/dri/by-path/pci-0000:01:00.0-*`), wyładowuje moduły NVIDIA (`modprobe -r nvidia_uvm nvidia_drm nvidia_modeset nvidia`), podpina obie funkcje pod vfio-pci (`virsh nodedev-detach`) i wznawia sesję (Plasma działa wtedy na iGPU); przy `release` robi odwrotnie (reattach + modprobe + restart sesji + start ollamy). Po powrocie GPU z prawdziwej sesji Windows (12.09 13:04) host zamroził się tuż po rebindzie: nieświeże węzły `/dev/nvidia*` mają nieaktualne numery major (`nvidia-uvm` dostaje major dynamicznie, a udev nie nadpisze istniejącego pliku). Release robi teraz: jawny reset VGA (`/sys/.../reset`) przed reattach, `rm -f /dev/nvidia*` przed modprobe, 3 s na stabilizację przed startem sesji i 5 s opóźnienia przed startem ollamy (CUDA). Drugi freeze (12.09 13:18, ~90 s po zwrocie GPU, dokładnie przy starcie nowej sesji po zalogowaniu) dał twardy trop: kernel logował `nvidia 0000:01:00.0: Enabling HDA controller` w momencie logowania — PipeWire otwierał HDMI audio dGPU, a KWin sesji dotykał świeżo wróconej karty (ollama/CUDA przetrwała wtedy 90 s, więc nie ona). Utrwalona architektura (12.09): release podpina z powrotem TYLKO VGA — funkcja audio zostaje na vfio-pci między cyklami (PipeWire jej nie widzi); sesja użytkownika jest pinowana do iGPU przez plasmowy env-script `~/.config/plasma-workspace/env/igpu-kwin.sh` (`export KWIN_DRM_DEVICES=/dev/dri/igpu-card`, tmpfiles); symlink `/dev/dri/igpu-card` tworzy reguła udev (DEVPATH — niezależna od kolejności reguł) + tmpfiles + awaryjnie sam hook; greeter celowo zostaje domyślny (boot identyczny). Węzły `/dev/nvidia*` po modprobe są odtwarzane deterministycznie w hooku (major uvm z `/proc/devices` — udev mknod EEXIST w wyścigu). NIGDY nie podawać `by-path` (dwukropki!) bezpośrednio do KWIN_DRM_DEVICES — KWin rozbija wartość po `:` i SIGSEGV w `KWin::VulkanDevice` (boot 12:05 2026-09-12). Degradacja bezpieczna: gdyby pinning nie zadziałał, hook ubija procesy trzymające GPU i restartuje sesję (stary scenariusz). Ollama: `services.ollama.package = pkgs.ollama-cuda` w `ai.nix` — domyślny pakiet nie ma w LD_LIBRARY_PATH runpath OpenGL (`/run/opengl-driver/lib`) i wykrywa tylko CPU (log `id=cpu`, 12.09 13:20). GPU hosta działa normalnie poza czasem życia VM (PRIME offload/CUDA); monitor na wyjściu dGPU pokazuje obraz dopiero po zwrocie GPU do hosta. Przykładowa definicja VM: `modules/hosts/nixos/win11-vm.xml` (UEFI `firmware=efi`, TPM swtpm, kvm hidden + hyperv vendor_id, 16 GiB RAM, 8 vCPU pinowanych do 4 P-core'ów, SATA disk, SPICE fallback + shmem looking-glass); instalacja: `qemu-img create -f qcow2 /var/lib/libvirt/images/win11.qcow2 128G` + `sudo virsh define modules/hosts/nixos/win11-vm.xml`; ISO instalacyjne wrzucaj do `/var/lib/libvirt/images/` (np. `sudo cp ~/Pobrane/Win11*.iso /var/lib/libvirt/images/win11.iso` + `chmod 644`) — katalog domowy ma tryb 700, więc `qemu-libvirtd` dostaje EACCES i VM od razu pada; aliasy `vm-start`/`vm-stop`/`vm-status`/`vm-console` (z jawnym `-c qemu:///system` — bez tego nie-root `virsh` łączy się z `qemu:///session` i nie widzi systemowych domen). Diagnostyka: `journalctl -t vfio-hook`, grupy IOMMU: `ls /sys/kernel/iommu_groups/*/devices/`. Uwaga: `virtualisation.libvirtd.qemu.ovmf` zostało USUNIĘTE z nixpkgs (OVMF dostępne domyślnie przez `<os firmware='efi'>`) — nie dodawać z powrotem.
* **Plasma Browser Integration**: single line `programs.chromium.enable = true` in `modules/hosts/nixos/packages.nix` — the nixpkgs option installs no browser, it only writes `/etc` entries (policies + native messaging manifests). This lets the `plasma6` module's own `enablePlasmaBrowserIntegration` wiring expose the `org.kde.plasma.browser_integration` manifest in `/etc/chromium` and `/etc/opt/chrome` for Chromium-based browsers installed outside `programs.chromium` (Helium from a local `.deb` package, ungoogled-chromium from `systemPackages`). Without it the "Plasma Integration" extension shows "Specified native messaging host not found". Firefox is wired automatically by the `plasma6` module (`programs.firefox.nativeMessagingHosts.packages`). The package itself (`kdePackages.plasma-browser-integration`) comes from the `plasma6` module.
...
* **Hermes Agent** (`modules/hosts/raspberry-pi-4/hermes.nix`): AI agent service (`services.hermes-agent`) on port 8642. Uses `gpt-6-sol` via `openai-codex` (Hermes OAuth login required as the `hermes` service user) with `agent.reasoning_effort = "xhigh"`; `glm-5.3-flash:cloud` via Ollama Cloud (`OLLAMA_API_KEY`) is the fallback model and remains the `auxiliary.vision` model. `ddgs` is the web backend. `require_approval = false`. `addToSystemPackages = true`. `restart = "always"`, `restartSec = 5`. Extra packages: `uv`, `nodejs_22`, `ripgrep`, `ffmpeg`, `python311`. Extra dependency groups: `all`, `messaging`, `matrix`. Configured MCP servers: `trilium-notes` (HTTP at `127.0.0.1:8081/mcp` with Bearer token) and `nixos` (`uvx mcp-nixos`). Environment loaded from `hermes-env.age` and `llmgateway-api-key.age` (agenix). Sudo NOPASSWD for user `hermes` (ALL). Service hardening overrides: `NoNewPrivileges = false` (enables sudo), `UMask = 0027` (group-readable files for `nixos` user in `hermes` group), `ExecStartPre` cleans stale lock/pid/state files. All decrypted Agenix secrets on the Pi are readable by the `users` group (`0440`), which includes `nixos` and `hermes`; the local `0600` override for `~/.hermes/.env` was removed. User `hermes` in groups: `users`, `keys`, `wheel`, `systemd-journal`, `disk`. User `nixos` added to `hermes` group.

...
* **Open WebUI** (`modules/hosts/raspberry-pi-4/open-webui.nix`): Open-source AI chat interface on port 3001 (localhost only), fronted by an **nginx reverse proxy on port 8080** (cloudflared ingress target) with static-asset caching (`proxy_cache`, immutable `Cache-Control` headers, gzip, response buffering). Connects to three OpenAI-compatible backends via semicolon-separated multi-endpoint lists (`ENABLE_OPENAI_API = "true"`, `OPENAI_API_BASE_URLS = "http://127.0.0.1:8642/v1;https://api.llmgateway.io/v1;http://127.0.0.1:8643/v1"`): index 0 = Hermes Agent (`127.0.0.1:8642/v1`), index 1 = LLM Gateway devpass (`api.llmgateway.io/v1`), index 2 = Prime Agent bridge (`127.0.0.1:8643/v1`). Keys paired by index in `OPENAI_API_KEYS` from the age-encrypted `open-webui-keys.age` (appended to the service via a second `systemd.serviceConfig.EnvironmentFile`, kept out of `environment {}` so they never enter the world-readable nix store). Ollama API disabled (`ENABLE_OLLAMA_API = "false"`). Auth required (`WEBUI_AUTH = "True"`). `ENABLE_PERSISTENT_CONFIG = "False"` (env vars override DB-stored values). Stateful Responses API enabled. Session/auth cookies set to `Secure` + `SameSite=none` (required behind Cloudflare Tunnel TLS termination). Shared Hermes API key via `hermes-env.age`. **Prime Agent bridge** (`modules/hosts/raspberry-pi-4/prime-agent-bridge.nix`): Prime Agent is a CLI with no built-in HTTP server, so the `prime-agent-bridge` package (a dependency-free stdlib-Python `ThreadingHTTPServer`, `modules/packages/_prime-agent-bridge/`) translates OpenAI `/v1/chat/completions` into one-shot `prime-agent --print --mode json` runs and listens on `127.0.0.1:8643` as systemd service `prime-agent-bridge`. Runs as dedicated system user `prime-bridge` with persistent writable home `/var/lib/prime-bridge` (for the lazily-seeded `~/.prime/agent/` config + IPython kernel-venv bootstrap via `uv`, which is added to PATH by the wrapper). `PRIME_BRIDGE_TIMEOUT=280` stays under open-webui's hard 300s request cap. No auth — localhost-only, same posture as Hermes. When a specific model is requested the bridge must pass both `--provider llmgateway` and `--model <id>` (model alone resolves to built-in OAuth providers, not the custom `models.json` provider). Advertised model ids are `prime-agent` (default) plus `prime-agent/<id>` for each seeded model.
* **Ollama**: Local LLM backend (`services.ollama.enable`), EnvironmentFile from `hermes-env.age`.
* **Web Terminal / ttyd** (`modules/hosts/raspberry-pi-4/ttyd.nix`): Browser terminal (`services.ttyd`) bound to `127.0.0.1:7681` (interface `lo` — no firewall ports opened, reachable only via the Cloudflare Tunnel ingress `terminal.${customTop.site.full}`). HTTP basic auth (`username = "janusz"`, password from agenix `ttyd-pass.age`, registered in `secrets.nix` as `mkSecret serverHosts`); writable; runs as user `nixos` (wheel → sudo works) with login shell `bash -l` (base profile execs fish for interactive sessions).
...
* **Cloudflared**: Tunnel to expose services externally (root and `chat.` hosts get `originRequest` tuning: `connectTimeout = 300s`, keepalive settings, to survive long photo uploads / slow LLM streams):
  ...
  * `terminal.${customTop.site.full}` -> ttyd web terminal (localhost:7681)

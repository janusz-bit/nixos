...
* **Plasma Browser Integration** (`modules/hosts/nixos/plasma-browser-integration.nix`): exposes the `org.kde.plasma.browser_integration` native-messaging manifest in `/etc/chromium` and `/etc/opt/chrome` (guarded by `!programs.chromium.enable` to avoid conflicts). The nixpkgs `plasma6` module only writes these manifests when `programs.chromium.enable` is set, so Chromium-based browsers installed outside `programs.chromium` (Helium from a local `.deb` package, ungoogled-chromium from `systemPackages`) could not find the host — the "Plasma Integration" extension showed "Specified native messaging host not found". Firefox is wired automatically by the `plasma6` module (`programs.firefox.nativeMessagingHosts.packages`). The package itself (`kdePackages.plasma-browser-integration`) comes from the `plasma6` module.
